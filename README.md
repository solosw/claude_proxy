...
### 💬 模型身份拦截
针对"你是什么模型"、"你叫什么"等常见身份类问题，直接从 Combo 的 Description 字段返回预设回复，无需调用上游 API，节省成本且响应更快。支持中文（你是什么模型/你叫什么/你是谁/你是啥）和英文（what model/who are you/what's your name）问法。
...
| 模型身份拦截 | ✅ | 常见身份问题直接返回 Description，节省成本 |
...
- `GET /back/v1/models` — 获取当前用户可用的 combo 模型列表，返回 OpenAI 兼容格式
